...
def _validate_secret_shape(secret: Any, declaration: dict[str, Any]) -> None:
    ...
    if type(secret["arn"]) is not str or not re.fullmatch(arn_pattern, secret["arn"]):
        raise ValueError("Observed secret identity differs")


def _validate_secret_version(version: Any) -> None:
    """Accept only native version identifiers, never values or arbitrary text."""
...
def _validate_references(desired: dict[str, Any], observed: dict[str, Any]) -> None:
    ...
    for purpose, declaration in desired.items():
        ...
        _validate_secret_version(secret["version_id"])
